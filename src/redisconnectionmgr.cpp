...
bool RedisConnectionMgr::getValue(const std::string& key, const std::string& threadId, std::string& value) {
    ...
    if (context == nullptr || context->err) {
        LOG_ERROR("redisconnectionmgr", 864, "getValue", threadId, "Could not connect to Redis");
        return false;
    }

    redisReply *reply = (redisReply *)redisCommand(context, "GET %s", key.c_str());
    if (reply == nullptr) {
        LOG_ERROR("redisconnectionmgr", 864, "getValue", threadId, "Error executing command");
        redisFree(context);
        return false;
    }
...
bool RedisConnectionMgr::setValue(const std::string& key, const std::string& value, const std::string& threadId) {
    redisContext *context = redisConnect("127.0.0.1", 6379);
    if (context == nullptr || context->err) {
        LOG_ERROR("redisconnectionmgr", 864, "setValue", threadId, "Could not connect to Redis");
        return false;
    }

    redisReply *reply = (redisReply *)redisCommand(context, "SET %s %s", key.c_str(), value.c_str());
    if (reply == nullptr) {
        LOG_ERROR("redisconnectionmgr", 864, "setValue", threadId, "Error executing command");
        redisFree(context);
        return false;
    }
